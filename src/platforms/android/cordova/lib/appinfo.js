...
function readAppInfoFromManifest() {
    ...
    if (!packageName) {
        throw new Error('Could not find package name within ' + manifestPath);
    }
    var activityTag = /<activity\b[\s\S]*<\/activity>/.exec(manifestData);
    if (!activityTag) {
        throw new Error('Could not find <activity> within ' + manifestPath);
    }
    var activityName = /\bandroid:name\s*=\s*"(.+?)"/.exec(activityTag);
    if (!activityName) {
        throw new Error('Could not find android:name within ' + manifestPath);
    }
