...
var shell = require('shelljs'), path = require('path'), fs = require('fs'), ROOT = path.join(__dirname, '..', '..');
...
module.exports.check_android = function () {
    ...
    if (targets.code > 0 && targets.output.match(/command\snot\sfound/)) {
        ...
        console.error('Please install Android target ' + valid_target.split('-')[1]
                          + ' (the Android newest SDK). Make sure you have the latest Android tools installed as well. Run \"android\" from your command-line to install/update any missing SDKs or tools.');
        return false;
    }
    return true;
}
