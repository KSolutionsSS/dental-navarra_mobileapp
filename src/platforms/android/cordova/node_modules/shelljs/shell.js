...
var fs = require('fs'), path = require('path'), util = require('util'), vm = require('vm'), child = require('child_process'), os = require('os');
...
function _cp(options, sources, dest) {
    ...
    var exists = fs.existsSync(dest), stats = exists && fs.statSync(dest);
    ...
    sources.forEach(function (src) {
        ...
        if (fs.statSync(src).isDirectory()) {
            if (!options.recursive) {
                ...
                var newDest = path.join(dest, path.basename(src)), checkDir = fs.statSync(src);
                try {
...
function _which(options, cmd) {
    ...
    var pathEnv = process.env.path || process.env.Path || process.env.PATH, pathArray = splitPath(pathEnv), where = null;
        ...
        if (isNaN(parseInt(mode, 8))) {
            ...
            mode.split(',').forEach(function (symbolicMode) {
                ...
                if (matches) {
                    ...
                    var mask = 0;
                    if (changeOwner) {
                        mask |= (changeRead ? PERMS.OWNER_READ : 0) + (changeWrite ? PERMS.OWNER_WRITE : 0) + (changeExec ? PERMS.OWNER_EXEC : 0)
                            + (changeSetuid ? PERMS.SETUID : 0);
                    }
                    if (changeGroup) {
                        mask |= (changeRead ? PERMS.GROUP_READ : 0) + (changeWrite ? PERMS.GROUP_WRITE : 0) + (changeExec ? PERMS.GROUP_EXEC : 0)
        ...
        try {
            ...
            if (options && options.notUnix) {
                ...
                if (args.length === 0 || typeof args[0] !== 'string' || args[0][0] !== '-') {
                    args.unshift('');
                } // only add dummy option if '-option' not already present
                retValue = fn.apply(this, args);
            }
        } catch (e) {
            if (!state.error) {
...
function copyFileSync(srcFile, destFile) {
    ...
    var BUF_LENGTH = 64 * 1024, buf = new Buffer(BUF_LENGTH), bytesRead = BUF_LENGTH, pos = 0, fdr = null, fdw = null;
...
function rmdirSyncRecursive(dir, force) {
    ...
    for (var i = 0; i < files.length; i++) {
        var file = dir + "/" + files[i], currFile = fs.lstatSync(file);
...
function tempDir() {
    ...
    state.tempDir = writeableDir(process.env['TMPDIR']) || writeableDir(process.env['TEMP']) || writeableDir(process.env['TMP'])
...
function execSync(cmd, opts) {
    var stdoutFile = path.resolve(tempDir() + '/' + randomFileName()), codeFile = path.resolve(tempDir() + '/'
                                                                                                   + randomFileName()), scriptFile = path.resolve(tempDir()
                                                                                                                                                      + '/'
                                                                                                                                                      + randomFileName()), sleepFile = path.resolve(tempDir()
                                                                                                                                                                                                        + '/'
                                                                                                                                                                                                        + randomFileName());
    ...
    var script = "var child = require('child_process')," + "     fs = require('fs');" + "child.exec('" + escape(cmd)
                     + "', {env: process.env, maxBuffer: 20*1024*1024}, function(err) {" + "  fs.writeFileSync('" + escape(codeFile)
                     + "', err ? err.code.toString() : '0');" + "});";
